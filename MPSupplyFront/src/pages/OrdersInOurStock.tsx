...
const OrderInOurStock = () => {
    ...
    const [fetchData, isLoading, errorData] = useFetching(async () => {
        const res = await ObservableRAPI.getInOurStock()
    ...
    async function saveStock() {
        ...
        orders.forEach(supplier => {
            supplier.items.forEach((item: any) => {
                items.push({
                    id: item.id,
                    inOurStock: item.inOurStock,
                    add: item.add
                })
            })
        })

        const res = await ObservableRAPI.setInOurStock(items)
    ...
    function setAdd(sindx: number, indx: number, add: number) {
        const orderCopy = [...orders]
        orderCopy[sindx].items[indx].add = add
    ...
    function clearAdd() {
        ...
        orderCopy.forEach((supplier, sindx) => {
            supplier.items.forEach((item: any, indx: number) => {
                orderCopy[sindx].items[indx].add = 0
            })
        })
    ...
    return (
        <div className="row">
            ...
            <div className="col-md-12">
                    {orders.filter((order: any) => order.items.filter((f: any) => f.name.toLowerCase().includes(search.toLowerCase())).length > 0).map((supplier, sindx: number) =>
                <div className="card dataTable-wrapper mt-4" key={sindx}>
                    <div className="table-responsive">
                        <table className="table align-items-center">
                            <thead>
                                <tr>
                                    <th data-del-colspan={3} colSpan={5} className="text-center">{supplier.name}</th>
                                </tr>
                                <tr>
                                    <th style={{width: '70%'}} className="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">Наименование</th>
                                    <th className="text-center text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">Цена за шт.</th>
                                    <th className="text-center text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">Текущие остатки</th>
                                    <th className="text-center text-uppercase text-secondary text-xxs font-weight-bolder opacity-7" data-del="true">Закуплено</th>
                                    <th className="text-center text-uppercase text-secondary text-xxs font-weight-bolder opacity-7" data-del="true">Итого</th>
                                </tr>
                            </thead>
                            <tbody>
                            {supplier.items.length > 0 && supplier.items.filter((f: any) => f.name.toLowerCase().includes(search.toLowerCase())).map((el: any, indx: number) =>
                                <tr key={indx}>
                                    ...
                                    <td className="text-center">
                                        <span style={{display: 'none'}}>{el.inOurStock}</span>
                                        <span data-del="true">{Intl.NumberFormat("ru").format(el.inOurStock)}</span>
                                    </td>
                                    <td className="text-center" data-del="true">
                                        ...
                                        value={el.add}
                                        onChange={(e) => setAdd(sindx, indx, parseInt(e.target.value))}
                                    />
                                    </td>
                                    <td className="text-center" data-del="true">
                                        <span style={{display: 'none'}}>{el.inOurStock + el.add}</span>
                                        <span data-del="true">{Intl.NumberFormat("ru").format(el.inOurStock + el.add)}</span>
                                    </td>
                                </tr>
                            )}
                ...
                <button className='btn bg-gradient-success float-end mt-4' onClick={saveStock}>Сохранить</button>
                <button className='btn bg-gradient-primary float-end mt-4 me-3' onClick={() => tableToXLSX('all')}>Скачать</button>
                <button className='btn bg-gradient-primary float-end mt-4 me-3' onClick={clearAdd}>Сбросить все</button>
            </div>
        </div>
    );
};

export default OrderInOurStock;