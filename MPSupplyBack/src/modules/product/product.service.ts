...
import { API_GO_HOSTS, MP, ProductAttributesResponseOzon, ProductsResponseWB, SETTINGS, SETTINGS_MODULE } from 'src/main.types';
    ...
    if (products) {
      ...
      if (productAttributes) {
        productAttributes.forEach((el) => {
          this.productRepository.update({ volume: this.getVolume(el) }, { where: { foreignId: el.id, cid, mp: MP.Ozon } });
        });
      }
    }
